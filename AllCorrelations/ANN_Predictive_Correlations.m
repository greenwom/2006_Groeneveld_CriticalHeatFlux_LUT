function ANN_Predictive_Correlations()
...
Toggle_Values = [Groeneveld_Toggle,Biasi_Toggle,EPRI_Toggle,...
                 CISEGE_Toggle,W3_Toggle,cwall,nu,WP_Toggle];
...
Sheet = 'Condensed LUT';
% Range = 'A126:J151';
Range = 'A2:J151'; %This is the full range of data
[Num,~] = xlsread(File,Sheet,Range);

Identifier_s = Num(:,1);  %Test identifying name
Inlet_sub_s = Num(:,2);   %Inlet Subcooling [kJ/kg]
Inlet_Press_s = Num(:,3).*(1e6); %Inlet Pressure [Pa]
Pr_s = Num(:,3)./22.064;    %Reduced Pressure [-]
G_flux_s = Num(:,4);        %Mass flux [kg/m^2-s]
h_fg_s = Num(:,10);         %Latent heat of Vaporization [kJ/kg]

% Delta_L_s = Num(:,20);    %Thermal Expansion Coefficient [m/m]
x_in_s = -Inlet_sub_s./h_fg_s;  %Inlet thermodynamic quality [-]

%Write selected data to Excel file
File_Save = strcat(pwd,'\COPYME_ANN_Correlations.xls');
...
Identifier = sprintf('ANN %0.0d',Identifier_s(test_num));
Inlet_Press = Inlet_Press_s(test_num);
Pr = Pr_s(test_num);
x_in = x_in_s(test_num);
G_flux = G_flux_s(test_num);
h_fg = h_fg_s(test_num).*1000;
h_fsat = IAPWS_IF97('hL_p',Inlet_Press/(1e6)).*1000;%Saturated enthalpy [J/kg]
rho_fsat = 1/IAPWS_IF97('vL_p',Inlet_Press/(1e6));  %saturated liquid density [kg/m^3]
rho_gsat = 1/IAPWS_IF97('vV_p',Inlet_Press/(1e6));  %vapor density (assumed at saturation) [kg/m^3]
...
% Nuclear Energy 1967, pp. 241-248.
    ...
    W3_Correlation(A_heated,A_subch,G_flux,h_fg,Inlet_Press,L_heated,...
                  x_in,h_fsat,D_e,toggle_nonuni,toggle_vis,...
                  toggle_plotsave,plotfilepath);
